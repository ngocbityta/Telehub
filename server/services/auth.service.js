...
const loginUser = async (identifier, password) => {
    ...
    const query = EMAIL_REGEX.test(identifier) ? { email: identifier } : { username: identifier };
    ...
    const accessToken = JWT.sign({ UserInfo: userInfo(user) }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: '8h' });
    const refreshToken = JWT.sign({ UserInfo: userInfo(user) }, process.env.REFRESH_TOKEN_SECRET, { expiresIn: '7d' });
...
const registerUser = async (username, email, password) => {
    ...
    const refreshToken = JWT.sign({ UserInfo: { username, email } }, process.env.REFRESH_TOKEN_SECRET, { expiresIn: '7d' });
    ...
    const accessToken = JWT.sign({ UserInfo: userInfo(newUser) }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: '8h' });
...
const googleLogin = async ({ email, name, picture }) => {
    ...
    if (!user) {
        ...
        user = new User({ username, email, fullname: name, password: hashedPassword, image: picture });
    }
